...
import abi from '../abis/genesis.json'
...
const createProject = async ({
  ...
  try {
    ...
    tx = await contract.createProject(title, description, imageURL, pdfURL, cost, expiresAt, QuadraticFundingRatio )
    await tx.wait()
    await loadProjects()
  } catch (error) {
    reportError(error)
...
const investProject = async (id, amount) => {
  try {
    if (!ethereum) return alert('Please install Metamask')
    const connectedAccount = getGlobalState('connectedAccount')
    const contract = await getEtheriumContract()
    ...
    tx = await contract.investProject(id, {
      from: connectedAccount,
      value: amount._hex,
    })

    await tx.wait()
    //await getInvestors(id)
  } catch (error) {
    reportError(error)
  }
}
...
const getInvestors = async (id) => {
  try {
    if (!ethereum) return alert('Please install Metamask')
    const contract = await getEtheriumContract()
    let investors = await contract.getInvestors(id)

    setGlobalState('investors', investors)
  } catch (error) {
    reportError(error)
  }
}
...
const structuredProjects = (projects) =>
  projects
    .map((project) => ({
      ...
      QuadraticFundingRatio: project.QuadraticFundingRatio.toNumber(),
...
const approveProject = async (id) => {
  try {
    ...
    // Call the approveProject method in the contract
    tx = await contract.approveProject(id, {
      from: connectedAccount,
    })

    await tx.wait()
    // Optionally, you can update the state or perform other actions after approval
  } catch (error) {
    reportError(error)
  }
}

const getApprovedAddresses = async (projectId) => {
  try {
    const contract = await getEtheriumContract();
    if (!contract) throw new Error('Ethereum contract not found.');

    const approvedAddresses = await contract.getApprovedAddresses(projectId);
